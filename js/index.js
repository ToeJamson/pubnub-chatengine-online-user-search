...
let config = { prop: 'state.full', caseSensitive: false }
...
const init = () => {
  ...
  ChatEngine.on('$.ready', function(data) {
      ...
      myChat = new ChatEngine.Chat('chatengine-demo-user-search');

      //  Start of user search plugin
      // ----------------------------

				myChat.plugin(ChatEngineCore.plugin['chat-engine-online-user-search'](config));
				
				let searchBar = $("#search-user")
				searchBar.on("input", function () {
					let searchTerm = searchBar.val()
					let results = myChat.onlineUserSearch.search(searchTerm);
					console.log(results)
					$("#people-list .list").html("")
					for (const user of results) {
						$('#people-list ul').append(peopleTemplate(user))	
					}
				})
